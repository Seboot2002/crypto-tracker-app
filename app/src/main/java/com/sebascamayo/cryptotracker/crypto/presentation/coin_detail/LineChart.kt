...
// Linea que aparece en el grafico

@Composable
fun LineChart(
    dataPoints: List<DataPoint>,
    style: ChartStyle, // Pasamos un estilo de letra
    visibleDataPointsIndices: IntRange, // rango que permite obtener cantidad especifica de dataPoints
    unit: String, // es la unidad de dinero
    modifier: Modifier = Modifier,
    selectedDataPoint: DataPoint? = null, // Punto seleccionado
    onSelectedDataPoint: (DataPoint) -> Unit = {}, // Funcion al seleccionar punto
    onXLabelWidthChange: (Float) -> Unit = {}, // Funcion de ajustar ancho de etiquetas de eje x
    showHelperLines: Boolean = true // Si se dibujan lineas horizontal y vertical
) {
    // Aplicamos este estilo de texto en el archivo
    val textStyle = LocalTextStyle.current.copy(
        fontSize = style.labelFontSize,
    )

    // se obtienen los points en un rango
    val visibleDataPoints =  remember(dataPoints, visibleDataPointsIndices) {
        dataPoints.slice(indices = visibleDataPointsIndices)
    }

    // Se obtienen el max y min de Y para ver la escala del grafico
    ...
    // Medicion de tamaño de texto
    // Esto sirve para calcular el tamaño en distintos estilos
    ...
    // Obtenemos index de Coin seleccionado
    ...
    // Se genera el dibujo mediante el objeto Canvas
    Canvas(
        modifier = modifier
            .fillMaxSize()
        ...
        // Obtenemos los datos en pixeles de los estilos: Dp to pixels
        ...
        // VIEWPORT: zona en  la que se dibuja la linea de valores
        // Obtenemos el area disponible y le restamos todos las distancias que ocupen espacio
        ...
        // El heigh y width es distinto a los puntos de coordenada como top o right
        // En este caso los puntos de coordenada son los objetos Offset
        val viewPortTopY = verticalPaddingPx + xLabelLineHeight + 10f
        val viewPortRightX = size.width
        val viewPortBottomY = viewPortTopY + viewPortHeightPx
        val viewPortLeftX = 2f * horizontalPaddingPx + maxYLabelWidth

        xLabelWidth = maxXLabelWidth + xAxisLabelSpacingPx // width completo del label en x

        // Obtenemos los textos e insertamos cada uno en el Canva
        xLabelTextLayoutResults.forEachIndexed { index, result ->
            ...
            // Mostrar label del punto seleccionado
